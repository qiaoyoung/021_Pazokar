...
- (instancetype) initWithFrame:(NSArray *(^)(void))data{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    ...
    return self.shouldAt.count;
}

//: - (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    //: NIMCommonTableSection *tableSection = self.data[section];
    ContentTagSection *tableSection = self.shouldAt[section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ContentTagSection *tableSection = self.shouldAt[indexPath.section];
    //: NIMCommonTableRow *tableRow = tableSection.rows[indexPath.row];
    PublicAppearRow *tableRow = tableSection.rows[indexPath.row];
    //: NSString *identity = tableRow.cellClassName.length ? tableRow.cellClassName : DefaultTableCell;
    NSString *identity = tableRow.cellClassName.length ? tableRow.cellClassName : app_kitPath;
    ...
    if (![cell respondsToSelector:@selector(should:position_strong:)]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!tableRow.forbidSelect) {
        ...
        UIViewController *vc = tableView.language;
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //: }else{
        ...
        ContentTagSection *section = self.shouldAt[indexPath.section];
...
- (void)by:(PublicAppearRow *)rowData eigenvalueOfASquareMatrix_strong:(UITableViewCell *)cell{
