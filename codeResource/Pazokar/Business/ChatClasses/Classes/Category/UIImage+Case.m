...
NSString *StringFromHypocorismData(Byte *data);        


//: default
Byte mainCellText[] = {50, 7, 41, 4, 59, 60, 61, 56, 76, 67, 75, 190};

//: content
Byte appDrawingMsg[] = {82, 7, 87, 11, 81, 18, 18, 100, 244, 254, 160, 12, 24, 23, 29, 14, 23, 29, 236};

//: Chartlet
Byte main_modeId[] = {86, 8, 18, 12, 145, 106, 218, 215, 68, 33, 249, 157, 49, 86, 79, 96, 98, 90, 83, 98, 14};
...
@implementation UIImage (Case)

//: + (UIImage *)nim_fetchChartlet:(NSString *)imageName chartletId:(NSString *)chartletId{
+ (UIImage *)userIndex:(NSString *)imageName chartlet:(NSString *)chartletId{
    //: if ([chartletId isEqualToString:@"default"]) {
    if ([chartletId isEqualToString:StringFromHypocorismData(mainCellText)]) {
        //: return [UIImage imageNamed:imageName];
        return [UIImage imageNamed:imageName];
    }
    //: NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",@"Chartlet",chartletId,@"content"];
    NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",StringFromHypocorismData(main_modeId),chartletId,StringFromHypocorismData(appDrawingMsg)];
    //先拿2倍图
...
- (UIImage *)pictureWithoutUpload: (CGFloat)suggestPixels
{
        ...
        (width / height > maxRatio || height / width > maxRatio))
    {
        ...
        return [self georgiaHomeBoy:maxPixels];
    }
    //: else
    else
    {
        //: return [self nim_scaleWithMaxPixels:suggestPixels];
        return [self georgiaHomeBoy:suggestPixels];
    }
}

//: - (UIImage *)nim_scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)georgiaHomeBoy: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self backgroundBy:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)nim_scaleToSize:(CGSize)newSize
- (UIImage *)backgroundBy:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self withChild:size];
}

//: - (UIImage *)nim_drawImageWithSize: (CGSize)size
- (UIImage *)withChild: (CGSize)size
{
...
- (UIImage *)mentalRepresentationMessage:(CGSize)targetSize
{
    // 裁剪两边
...
Byte * HypocorismDataToCache(Byte *data) {
    int forbidVirus = data[0];
    int protectEating = data[1];
    Byte forSale = data[2];
    int totallyMin = data[3];
    if (!forbidVirus) return data + totallyMin;
    for (int i = totallyMin; i < totallyMin + protectEating; i++) {
        int value = data[i] + forSale;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[totallyMin + protectEating] = 0;
    return data + totallyMin;
}

NSString *StringFromHypocorismData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)HypocorismDataToCache(data)];
}
