...
static NSString* const mReportFormat = @"\u2026";

//: @interface M80AttributedLabel ()
@interface CypherScrollView ()
...
@implementation CypherScrollView
...
- (NSAttributedString *)bubblePressed:(NSString *)text
{
    //: if ([text length])
    if ([text length])
    {
...
- (NSAttributedString *)immobilize:(NSMutableAttributedString *)drawString
{
    //: if (drawString)
    if (drawString)
    {
...
- (NSMutableAttributedString *)showText:(NSAttributedString *)attributedString {
    // 匹配 [表情]
    ...
    if (attrM.length > 0) {
        ...
        NSArray<NSTextCheckingResult *> *emoticonResults = [[CypherScrollView of] matchesInString:attrM.string options:kNilOptions range:attrM.yy_rangeOfAll];
...
- (NSMutableAttributedString*)by:(NSMutableAttributedString *)wholeText {
...
- (void)setText:(NSString *)text
{
    ...
    NSAttributedString *attributedText = [self bubblePressed:text];
...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
    ...
    NSAttributedString *attString = [self immobilize:[[NSMutableAttributedString alloc] initWithAttributedString:attributedText]];
    //: _attributedString = [self needHightText:[self transformEmojiDescToEomjiImageWithAttributedString:attString]];
    _attributedString = [self by:[self showText:attString]];
...
- (void)canName:(NSString *)text
{
    //: NSAttributedString *attributedText = [self attributedString:text];
    NSAttributedString *attributedText = [self bubblePressed:text];
    //: [self appendAttributedText:attributedText];
    [self exceptState:attributedText];
}

//: - (void)appendAttributedText:(NSAttributedString *)attributedText
- (void)exceptState:(NSAttributedString *)attributedText
{
