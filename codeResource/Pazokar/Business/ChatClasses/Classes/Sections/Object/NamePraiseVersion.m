...
@property (nonatomic,weak) id<StigmatizePlayer> delegate;
                         ...
                         indexConfig:(id<QuantityelligenceInformationCan>)sessionConfig
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(outOfDoors:) name:UIMenuControllerDidHideMenuNotification object:nil];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ating:) name:notiMakeIdent object:nil];
    }
    //: return self;
    return self;
}
...
- (void)collection
{
    ...
    self.tableView.frame = CGRectMake(0, (44.0f + [UIDevice statusOrLevel]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice statusOrLevel])-self.inputView.height);
    //: self.tableView.bottom = self.inputView.nim_top;
    self.tableView.bottom = self.inputView.nim_top;
    //: [self.tableView nim_scrollToBottom:YES];
    [self.tableView light:YES];
...
- (void)routeMessage
{
    ...
    [self.refreshControl addTarget:self action:@selector(nameWindow:) forControlEvents:UIControlEventValueChanged];

}

//: - (void)headerRereshing:(id)sender
- (void)nameWindow:(id)sender
{
    //: if ([self.delegate respondsToSelector:@selector(onRefresh)])
    if ([self.delegate respondsToSelector:@selector(restoreShow)])
    {
        //: [self.delegate onRefresh];
        [self.delegate restoreShow];
    }
}

//: - (void)insert:(NSArray<NSIndexPath *> *)indexPaths animated:(BOOL)animated
- (void)informationAnimated:(NSArray<NSIndexPath *> *)indexPaths title:(BOOL)animated
{
    //: if (!indexPaths.count)
    if (!indexPaths.count)
    {
...
//        }];
    ...
    if (isScroll || [self thread:addIndexPathes])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
    }
...
- (void)cellDisable:(NSArray<NSIndexPath *> *)indexPaths
{
...
- (void)inputDoing {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!self.inputView.replyedContent.hidden)
        {
            ...
            [self.inputView.replyedContent alongOrientationDismiss];
        }
    //: });
    });
}
...
- (BOOL)thread:(NSArray<NSIndexPath *> *)indexPaths
{
