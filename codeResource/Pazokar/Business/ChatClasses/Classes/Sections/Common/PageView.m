...
- (NSInteger)towardMember:(NSInteger)value message:(NSInteger)min remote:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)userPage:(NSInteger)currentPage
{
    ...
    NSInteger first = [self towardMember:currentPage - 1 message:0 remote:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self towardMember:currentPage + 1 message:0 remote:count - 1];
