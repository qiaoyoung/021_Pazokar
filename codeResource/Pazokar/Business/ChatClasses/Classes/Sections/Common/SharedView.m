...
+ (instancetype)location:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self pullBack:rect tap:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self tinkleIndex:rect showContextRef:context];
    }
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self divertStr:badgeValue];
...
- (CGSize)latitude:(NSString *)badgeValue{
...
- (CGRect)divertStr:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self latitude:badgeValue];
...
- (void)pullBack:(CGRect)rect tap:(CGContextRef)context{
...
- (void)tinkleIndex:(CGRect)rect showContextRef:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
