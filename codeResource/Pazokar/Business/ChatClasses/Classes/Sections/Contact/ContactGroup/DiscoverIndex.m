...
@interface CleanAppear : NSObject
...
@implementation CleanAppear

//: - (instancetype)initWithFirst:(id)first second:(id)second {
- (instancetype)initWithColor:(id)first titleTarget:(id)second {
    //: self = [super init];
    self = [super init];
    //: if(self) {
    if(self) {
...
- (void)setMembers:(NSArray *)members {
    ...
    for (id<DatePraise>member in members) {
        ...
        NSString *groupTitle = [member end];
    ...
    [tmp enumerateKeysAndObjectsUsingBlock:^(NSString *groupTitle, NSMutableArray *groupedMembers, BOOL *stop) {
        ...
        if (groupTitle.length) {
            ...
            [_groups addObject:[[CleanAppear alloc] initWithColor:groupTitle titleTarget:groupedMembers]];
        }
    //: }];
    }];
    //: [self sortGroup];
...
- (void)setSpecialMembers:(NSArray *)specialMembers {
    ...
    if (specialMembers.count > 0) {
        ...
        [_groups insertObject:[[CleanAppear alloc] initWithColor:@"$" titleTarget:specialMembers] atIndex:0];
        //: [self sortGroupMember];
        [self sortOrAssociateMember];
...
- (NSDictionary *)contentDic {
    ...
    for (int i = 0; i < _groups.count; ++i) {
        ...
        NSArray *tempArr = [self range:i];
        //: NSString *title = [self titleofGroup:i];
        NSString *title = [self titleof:i];
        //: [tempArr enumerateObjectsUsingBlock:^(id<NIMGroupMemberProtocol>member, NSUInteger idx, BOOL * _Nonnull stop) {
        [tempArr enumerateObjectsUsingBlock:^(id<DatePraise>member, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSString *)titleof:(NSInteger)groupIndex {
...
- (NSArray *)range:(NSInteger)groupIndex
{
    //: if(groupIndex >= 0 && groupIndex < _groups.count) {
    if(groupIndex >= 0 && groupIndex < _groups.count) {
        //: NIMKitPair *pair = [_groups objectAtIndex:groupIndex];
        CleanAppear *pair = [_groups objectAtIndex:groupIndex];
        //: return pair.second;
        return pair.second;
    }
    //: return nil;
...
- (void)date
{
    ...
    [_groups sortUsingComparator:^NSComparisonResult(CleanAppear *pair1, CleanAppear *pair2) {
...
- (void)sortOrAssociateMember
{
    //: [_groups enumerateObjectsUsingBlock:^(NIMKitPair *obj, NSUInteger idx, BOOL *stop) {
    [_groups enumerateObjectsUsingBlock:^(CleanAppear *obj, NSUInteger idx, BOOL *stop) {
        ...
        [groupedMembers sortUsingComparator:^NSComparisonResult(id<DatePraise> member1, id<DatePraise> member2) {
            //: return _groupMemberComparator([member1 sortKey], [member2 sortKey]);
            return _groupMemberComparator([member1 meanSolarDay], [member2 meanSolarDay]);
        //: }];
        }];
    //: }];
    }];
