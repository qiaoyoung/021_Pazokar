...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(mying:));
...
+ (UIImage *)mying:(NSString *)name {
    ...
    UIImage *image = [[ValuePraise show] mying:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self mying:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
