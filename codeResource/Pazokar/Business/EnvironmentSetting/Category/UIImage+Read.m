...
@interface SizeSpouseData : NSObject

+ (instancetype)sharedInstance;

//: jpeg
@property (nonatomic, copy) NSString *noti_situationStr;

//: yyyy-MM-dd HH:mm:ss
@property (nonatomic, copy) NSString *app_trulyId;

//: tiff
@property (nonatomic, copy) NSString *kInfoAbsoluteName;

//: png
@property (nonatomic, copy) NSString *noti_procedureKey;

//: gif
@property (nonatomic, copy) NSString *dreamMessageStr;

@end

@implementation SizeSpouseData

+ (instancetype)sharedInstance {
    static SizeSpouseData *instance = nil;
...
+ (NSData *)SizeSpouseDataToData:(NSString *)value {
...
- (Byte *)SizeSpouseDataToCache:(Byte *)data {
    int side = data[0];
    Byte ade = data[1];
    int confinement = data[2];
    for (int i = confinement; i < confinement + side; i++) {
        int value = data[i] - ade;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[confinement + side] = 0;
    return data + confinement;
}

- (NSString *)StringFromSizeSpouseData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self SizeSpouseDataToCache:data]];
}

//: gif
- (NSString *)dreamMessageStr {
    if (!_dreamMessageStr) {
		NSString *origin = @"03420925b82730c663a9aba85a";
		NSData *data = [SizeSpouseData SizeSpouseDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _dreamMessageStr = [self StringFromSizeSpouseData:value];
    }
    return _dreamMessageStr;
}

//: yyyy-MM-dd HH:mm:ss
- (NSString *)app_trulyId {
    if (!_app_trulyId) {
		NSString *origin = @"13610b7eb7b902d26dbfc4dadadada8eaeae8ec5c581a9a99bcece9bd4d43f";
		NSData *data = [SizeSpouseData SizeSpouseDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _app_trulyId = [self StringFromSizeSpouseData:value];
    }
    return _app_trulyId;
}
...
- (NSString *)noti_situationStr {
    if (!_noti_situationStr) {
		NSString *origin = @"041c0b4943c4096c7d2612868c8183d5";
		NSData *data = [SizeSpouseData SizeSpouseDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _noti_situationStr = [self StringFromSizeSpouseData:value];
    }
    return _noti_situationStr;
}

//: tiff
- (NSString *)kInfoAbsoluteName {
    if (!_kInfoAbsoluteName) {
		NSString *origin = @"041f09a7e7475f90cd938885852f";
		NSData *data = [SizeSpouseData SizeSpouseDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _kInfoAbsoluteName = [self StringFromSizeSpouseData:value];
    }
    return _kInfoAbsoluteName;
}

//: png
- (NSString *)noti_procedureKey {
    if (!_noti_procedureKey) {
		NSString *origin = @"034905310fb9b7b0a0";
		NSData *data = [SizeSpouseData SizeSpouseDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _noti_procedureKey = [self StringFromSizeSpouseData:value];
    }
    return _noti_procedureKey;
}
...
- (UIImage *)titleShare:(BOOL)isHorizontal {
...
- (UIImage *)cast:(CGFloat)width {
    ...
    return [self tip:width disable:height];
}
...
- (UIImage *)message:(CGFloat)heigh {
    ...
    return [self tip:width disable:heigh];
}
...
- (UIImage *)quantityernalRepresentationDecoded {
...
- (UIImage *)colorDown {
    ...
    [df setDateFormat:[SizeSpouseData sharedInstance].app_trulyId];
    ...
    return [self cell:dateString
               //: textColor:[UIColor blackColor]
               nim:[UIColor blackColor]
                    //: font:[UIFont boldSystemFontOfSize:16.0f]
                    addHidden:[UIFont boldSystemFontOfSize:16.0f]
                   //: point:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
                   message:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
...
+ (NSString *) collectionWithinMetadata:(NSData *)data{
    ...
    switch (c) {
        ...
        case 0xFF:
            //: return @"jpeg";
            return [SizeSpouseData sharedInstance].noti_situationStr;
        //: case 0x89:
        case 0x89:
            //: return @"png";
            return [SizeSpouseData sharedInstance].noti_procedureKey;
        //: case 0x47:
        case 0x47:
            //: return @"gif";
            return [SizeSpouseData sharedInstance].dreamMessageStr;
        //: case 0x49:
        case 0x49:
        //: case 0x4D:
        case 0x4D:
            //: return @"tiff";
            return [SizeSpouseData sharedInstance].kInfoAbsoluteName;
    }
    //: return @"";
    return @"";
}
...
CGFloat localRadians(CGFloat degrees) {return degrees * 3.14159265358979323846264338327950288 / 180;};
//: CGFloat KKRadiansToDegrees(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
CGFloat restTable(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
...
- (UIImage *)yore:(CGFloat)degrees{
    ...
    CGAffineTransform t = CGAffineTransformMakeRotation(localRadians(degrees));
    ...
    CGContextRotateCTM(bitmap, localRadians(degrees));
...
- (UIImage*)origin:(CGSize)targetSize
{
    //: UIImage *sourceImage = self;
    UIImage *sourceImage = self;
    //: UIImage *newImage = nil;
...
- (UIImage *)name:(NSUInteger)maxLength{
    ...
    NSData *compressData = [self listInfo:maxLength*1024];
...
-(NSData *)listInfo:(double)maxLength{
    // Compress by quality
