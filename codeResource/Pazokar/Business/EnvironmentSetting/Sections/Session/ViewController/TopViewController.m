...
- (NSString *)appLetterMessage {
    ...
    if (!appLetterMessage) {
		NSString *origin = @"B2F5EDB2E4C9BFDBusyB3C3DEB2F2DA30";
		NSData *data = [YaData YaDataToData:origin];
...
NIMEventSubscribeManagerDelegate,
//: BusyTeamCardViewControllerDelegate,
ImageTop,
...
//: #pragma mark - BusyTeamCardViewControllerDelegate
...
- (void)inputTap:(NIMMessage *)message
{
    ...
    if (_intervalTime > 0 && _isSend == NO) {
        ...
        //: NSString *title = [NSString stringWithFormat:[BusyLanguageManager getTextWithKey:@"发言频次过快，请%ld秒后重试"],(long)_intervalTime];
...
//: #pragma mark - 菜单
#pragma mark - 菜单
//: - (void)onTapMenuItemReply:(BusyMediaItem *)item
- (void)onTapMenuItemReply:(ModelMessage *)item
{
...
//: - (void)onTapMenuItemForword:(BusyMediaItem *)item
- (void)onTapMenuItemForword:(ModelMessage *)item
{
...
//: - (void)onTapMenuItemMark:(BusyMediaItem *)item
- (void)onTapMenuItemMark:(ModelMessage *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        //: if (error) {
        if (error) {
            //: [SVProgressHUD showErrorWithStatus:[BusyLanguageManager getTextWithKey:@"收藏失败"]];
            [SVProgressHUD showErrorWithStatus:[SendName streetSmart:[[YaData sharedInstance] app_leadershipSpouseShowValue]]];
            //: return;
            return;
        }
        //: [SVProgressHUD showSuccessWithStatus:[BusyLanguageManager getTextWithKey:@"收藏成功"]];
        [SVProgressHUD showSuccessWithStatus:[SendName streetSmart:[[YaData sharedInstance] notiNearlySliceFormat]]];
    //: }];
    }];
}

//: - (void)onTapMenuItemPin:(BusyMediaItem *)item
- (void)onTapMenuItemPin:(ModelMessage *)item
{
    //: NIMMessage *message = [self messageForMenu];
    NIMMessage *message = [self messageForMenu];
...
//: - (void)onTapMenuItemUnpin:(BusyMediaItem *)item
- (void)onTapMenuItemUnpin:(ModelMessage *)item
{
    //: NIMMessage *message = [self messageForMenu];
    NIMMessage *message = [self messageForMenu];
...
//: - (void)onTapMenuItemRevoke:(BusyMediaItem *)item
- (void)onTapMenuItemRevoke:(ModelMessage *)item
{
    //: NIMMessage *message = [self messageForMenu];
    NIMMessage *message = [self messageForMenu];
...
//: - (void)onTapMenuItemReport:(BusyMediaItem *)item
- (void)onTapMenuItemReport:(ModelMessage *)item
{
...
//: - (void)onTapMenuItemTranslation:(BusyMediaItem *)item
- (void)onTapMenuItemTranslation:(ModelMessage *)item
{
    //: NIMMessage *message = [self messageForMenu];
    NIMMessage *message = [self messageForMenu];
...
- (void)ironedAlert:(NIMMessage *)message {
    ...
    __weak typeof(self) weakSelf = self;
    //: UIAlertAction *sure = [UIAlertAction actionWithTitle:[BusyLanguageManager getTextWithKey:@"contact_tag_fragment_sure"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)messageWill:(NIMMessage *)message packet:(NSString *)postscript{
    ...
    NIMRevokeMessageOption *option = [[NIMRevokeMessageOption alloc] init];
    //: option.apnsContent = [BusyLanguageManager getTextWithKey:@"撤回一条消息"];
    ...
    [[NIMSDK sharedSDK].chatManager revokeMessage:message option:option completion:^(NSError * _Nullable error) {
        ...
        if (error) {
            ...
            if (error.code == NIMRemoteErrorCodeDomainExpireOld) {
                //: UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:[BusyLanguageManager getTextWithKey:@"发送时间超过2分钟的消息不能被撤回"] delegate:nil cancelButtonTitle:[BusyLanguageManager getTextWithKey:@"contact_tag_fragment_sure"] otherButtonTitles:nil, nil];
            ...
            //: } else {
                ...
                //: [weakSelf.view makeToast:[BusyLanguageManager getTextWithKey:@"user_profile_avtivity_user_info_update_failed"] duration:2.0 position:CSToastPositionCenter];
...
- (void)tags:(NSNotification *)note {
    ...
    //: if (message) {
    if (message) {
        //: BusyMessageModel *model = [self uiDeleteMessage:message];
...
- (void)onTapMenuItemDelete:(ModelMessage *)item
{
    //: NIMMessage *message = [self messageForMenu];
    NIMMessage *message = [self messageForMenu];
...
//: - (void)onTapMenuItemMutiSelect:(BusyMediaItem *)item
- (void)onTapMenuItemMutiSelect:(ModelMessage *)item
{
    //: [self switchUIWithSessionState:NIMKitSessionStateSelect];
    [self maxState:NIMKitSessionStateSelect];
}

//: - (void)onTapMenuItemAudio2Text:(BusyMediaItem *)item
- (void)onTapMenuItemAudio2Text:(ModelMessage *)item
{
...
{
    //: [self.view makeToast:[BusyLanguageManager getTextWithKey:@"luyinshibai"] duration:2 position:CSToastPositionCenter];
...
- (void)object
{
    //: [self.view makeToast:[BusyLanguageManager getTextWithKey:@"luyintaiduan"] duration:0.2f position:CSToastPositionCenter];
...
- (BOOL)replyEnable:(DirectionEvent *)event
{
    ...
    //: if ([eventName isEqualToString:BusyKitEventNameTapContent])
    ...
    //: else if ([eventName isEqualToString:BusyKitEventNameTapRepliedContent])
...
//        [self.navigationController pushViewController:vc animated:YES];
    ...
    //: else if([eventName isEqualToString:BusyKitEventNameTapLabelLink])
...
- (void)margin:(NIMTeam *)team {
    ...
    //: } else {
        ...
        //: UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:[BusyLanguageManager getTextWithKey:@"activity_friend_verify_info"] delegate:nil cancelButtonTitle:[BusyLanguageManager getTextWithKey:@"contact_tag_fragment_sure"] otherButtonTitles:nil, nil];
...
- (void)on:(NSString *)message team:(NIMTeam *)team {
    ...
    [StraddleAlert view].config
        //: .LeeAddTitle(^(UILabel *label) {
        .LeeAddTitle(^(UILabel *label) {
            //: NSString *msg = [NSString stringWithFormat:@"%@: %@",[BusyLanguageManager getTextWithKey:@"message_helper_apply_to_group"],team.teamName];
        ...
        .LeeAddAction(^(FormatRedApp *action) {
            ...
            //: action.type = LEEActionTypeCancel;
            action.type = LEEActionTypeCancel;
            //: action.title = [BusyLanguageManager getTextWithKey:@"contact_tag_fragment_cancel"];
...
- (NSArray *)size:(NIMMessage *)message
{
    ...
    if ([DeviceDate valueForwarded:message]) {
        //: [items addObject:[[UIMenuItem alloc] initWithTitle:[BusyLanguageManager getTextWithKey:@"转发"] action:@selector(forwardMessage:)]];
    ...
    if ([DeviceDate sharedComprise:message]) {
        //: [items addObject:[[UIMenuItem alloc] initWithTitle:[BusyLanguageManager getTextWithKey:@"撤回"] action:@selector(revokeMessage:)]];
...
- (void)omit:(void (^)(NIMSession *targetSession))completion {
    //: UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:[BusyLanguageManager getTextWithKey:@"选择会话类型"] delegate:nil cancelButtonTitle:[BusyLanguageManager getTextWithKey:@"contact_tag_fragment_cancel"] destructiveButtonTitle:nil otherButtonTitles:[BusyLanguageManager getTextWithKey:@"watch_multiretweet_activity_person"], [BusyLanguageManager getTextWithKey:@"contact_fragment_group"], [BusyLanguageManager getTextWithKey:@"message_super_team"], nil];
...
- (void)successMessage:(id)sender
{
                                         ...
                                         completion:^(NSError * _Nullable error)
    {
        ...
        //: } else {
            ...
            //: BusyMessageModel *model = [weakSelf uiDeleteMessage:message];
 ...
 - (void)speechReadMessage:(NIMMessage *)message ginMill:(NIMSession *)session
{
    ...
    //: NSString *tip = [NSString stringWithFormat:@"%@ %@ ?", [BusyLanguageManager getTextWithKey:@"watch_multiretweet_activity_confirm_forwarded_to"], name];
...
- (BOOL)analogDigitalConverter
{
    ...
    if (![[Reachability reachabilityForInternetConnection] isReachable])
    {
        //: [self.view makeToast:[BusyLanguageManager getTextWithKey:@"friend_verify_avtivity_net_error"] duration:2.0 position:CSToastPositionCenter];
    ...
    if (self.session.sessionType == NIMSessionTypeTeam)
    {
        ...
        if (memberNumber < 2)
        {
            //: [self.view makeToast:[BusyLanguageManager getTextWithKey:@"app_avchat_not_start_with_less_member"] duration:2.0 position:CSToastPositionCenter];
...
- (UILabel *)invalid_tip {
    ...
    if (!_invalid_tip) {
        ...
        //: _invalid_tip.text = [BusyLanguageManager getTextWithKey:@"app_team_invalid_tip"]; 
...
//    self.reprotBlackView.userID = self.userId;
    ...
    self.reprotBlackView.speiceBackBlock = ^(NSString *Name){
        ...
        [[NIMSDK sharedSDK].userManager addToBlackList:self.userId completion:^(NSError *error) {
                   ...
                   //: if (!error) {
                   if (!error) {
                       //: [self.view makeToast:[BusyLanguageManager getTextWithKey:@"group_chat_avatar_activity_add_black_success"] duration:2.0f position:CSToastPositionCenter];
                   ...
                   //: }else{
                       ...
                       //: [self.view makeToast:[BusyLanguageManager getTextWithKey:@"black_list_activity_add_black_failed"] duration:2.0f position:CSToastPositionCenter];
...
//    self.reprotDeleteView.userID = self.userId
    ...
    self.reprotDeleteView.speiceBackBlock = ^(NSString *Name){
        ...
        [[NIMSDK sharedSDK].userManager deleteFriend:self.userId
            ...
            //: if (!error) {
            if (!error) {
                //: [self.view makeToast:[BusyLanguageManager getTextWithKey:@"contact_tag_fragment_delete_success"] duration:2.0f position:CSToastPositionCenter];
            ...
            //: }else{
                ...
                //: [self.view makeToast:[BusyLanguageManager getTextWithKey:@"friend_delete_fail"] duration:2.0f position:CSToastPositionCenter];
