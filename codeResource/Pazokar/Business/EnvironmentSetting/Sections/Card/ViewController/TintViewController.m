...
@interface TintViewController ()

//: @property (nonatomic,strong) NSString *userId;
@property (nonatomic,strong) NSString *userId;

//: @property (nonatomic,strong) NTESColorButton *chatButton;
@property (nonatomic,strong) ValuePraiseControl *chatButton;

//: @end
@end

//: @implementation NTESRobotCardViewController
@implementation TintViewController

//: - (instancetype)initWithUserId:(NSString *)userId
- (instancetype)initWithVertical:(NSString *)userId
{
...
- (void)viewDidLoad {
    ...
    self.chatButton = [[ValuePraiseControl alloc] initWithFrame:CGRectZero];
...
//: - (void)chat:(id)sender
- (void)latchkeyUp:(id)sender
{
    //: NIMSession *session = [NIMSession session:self.userId type:NIMSessionTypeP2P];
    NIMSession *session = [NIMSession session:self.userId type:NIMSessionTypeP2P];
    //: NTESSessionViewController *vc = [[NTESSessionViewController alloc] initWithSession:session];
    TopViewController *vc = [[TopViewController alloc] initWithSession:session];
