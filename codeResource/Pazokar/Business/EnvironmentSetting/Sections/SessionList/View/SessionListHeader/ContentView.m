...
//: #ECEEF2
@property (nonatomic, copy) NSString *user_redMsg;
...
+ (NSData *)RecentDataToData:(NSArray<NSNumber *> *)value {
...
- (Byte *)RecentDataToCache:(Byte *)data {
    int upright = data[0];
    Byte unburden = data[1];
    int labelVoice = data[2];
    for (int i = labelVoice; i < labelVoice + upright; i++) {
        int value = data[i] + unburden;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[labelVoice + upright] = 0;
    return data + labelVoice;
}
...
//: #ECEEF2
- (NSString *)user_redMsg {
    if (!_user_redMsg) {
		NSArray<NSNumber *> *origin = @[@7, @82, @7, @251, @18, @198, @55, @209, @243, @241, @243, @243, @244, @224, @203];
		NSData *data = [RecentData RecentDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _user_redMsg = [self StringFromRecentData:value];
    }
    return _user_redMsg;
}
...
//        customTitleLabel.font = [UIFont boldSystemFontOfSize:13];
        ...
        separaterView.backgroundColor = [UIColor status:[RecentData sharedInstance].user_redMsg];
