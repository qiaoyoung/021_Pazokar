...
@interface SoundControl : UIButton
...
- (void)with:(NIMMessageModel *)data
{
    //: [super refresh:data];
    [super refresh:data];
    //: [self setupRobot:data];
    [self chemicalAction:data];
}


//: - (UIImage *)chatBubbleImageForState:(UIControlState)state outgoing:(BOOL)outgoing
- (UIImage *)progressView:(UIControlState)state statusOf:(BOOL)outgoing
{
    //: return nil;
    return nil;
}
...
- (void)background:(NIMMessageModel *)data
{
...
- (void)chemicalAction:(NIMMessageModel *)data
{
...
- (void)at:(NIMKitRobotTemplateItem *)item
           //: inView:(UIView *)view
           accumulation:(UIView *)view
{
    ...
    switch (item.itemType) {
        ...
        case NIMKitRobotTemplateItemTypeText:
        {
            ...
            CypherScrollView *label = [self genLabel];
...
- (CGSize)minValue:(CGSize)size
{
    //: CGFloat height = [NTESSessionRobotContentView itemSpacing];
    CGFloat height = [ValueView itemSpacing];
    //: CGFloat width = 0;;
    CGFloat width = 0;;
    ...
    for (UIView *view in self.subviews) {
        ...
        if (view == self.continueButton)
        {
            ...
            height += [ValueView continueItemSpacing];
        }
        //: else
        else
        {
            //: height += [NTESSessionRobotContentView itemSpacing];
            height += [ValueView itemSpacing];
        }
    }
...
- (void)importantsed:(SoundControl *)button
{
    //: NIMKitEvent *event = [[NIMKitEvent alloc] init];
    NIMKitEvent *event = [[NIMKitEvent alloc] init];
    ...
    else
    {
        ...
        for (CypherScrollView *label in button.subviews)
        {
            //: if ([label isKindOfClass:[M80AttributedLabel class]])
            if ([label isKindOfClass:[CypherScrollView class]])
            {
...
- (void)imageFile
{
    ...
    [self classPerspective:self user:self.model.contentViewInsets];

    //: UIEdgeInsets contentInsets = self.model.contentViewInsets;
    UIEdgeInsets contentInsets = self.model.contentViewInsets;
    //: CGFloat top = [NTESSessionRobotContentView itemSpacing] + contentInsets.top;
    CGFloat top = [ValueView itemSpacing] + contentInsets.top;
    //: for (UIView *view in self.subviews)
    for (UIView *view in self.subviews)
    {
        //: if (view == self.bubbleImageView)
        ...
        top = view.bottom + [ValueView itemSpacing];
    }
}


//: - (void)resizeAllSubView:(UIView *)superView insets:(UIEdgeInsets)insets
- (void)classPerspective:(UIView *)superView user:(UIEdgeInsets)insets
{
    ...
    for (UIView *subView in superView.subviews)
    {
        ...
        if (subView.height == 0)
        {
            ...
            if ([subView isKindOfClass:[CypherScrollView class]])
            {
                //: M80AttributedLabel *label = (M80AttributedLabel *)subView;
                CypherScrollView *label = (CypherScrollView *)subView;
            ...
            else if ([subView isKindOfClass:[SoundControl class]])
            {
                //: NTESSessionRobotButton *button = (NTESSessionRobotButton *)subView;
                SoundControl *button = (SoundControl *)subView;
                //: button.width = width;
                button.width = width;
...
- (void)container:(UIView *)view
{
    ...
    for (UIView *subView in view.subviews)
    {
        ...
        if ([subView isKindOfClass:[SoundControl class]])
        {
            //: NTESSessionRobotButton *btn = (NTESSessionRobotButton *)subView;
            SoundControl *btn = (SoundControl *)subView;
            //: btn.target = nil;
            btn.target = nil;
            //: btn.url = nil;
            btn.url = nil;
...
- (CypherScrollView *)criticise
{
    //: M80AttributedLabel *label = nil;
    CypherScrollView *label = nil;
    ...
    else
    {
        ...
        label = [[CypherScrollView alloc] initWithFrame:CGRectZero];
...
- (SoundControl *)text
{
    ...
    else
    {
        ...
        button = [[SoundControl alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
