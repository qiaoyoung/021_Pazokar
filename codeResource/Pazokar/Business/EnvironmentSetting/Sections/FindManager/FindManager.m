...
NSString *const user_pointSubMessage = @"online_state";
...
@interface FindManager()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
+ (instancetype)value
{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        instance = [[FindManager alloc] init];
    //: });
    });
    //: return instance;
    return instance;
...
- (void)nearBar:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self with];
...
- (void)sign
{
    ...
    NSDictionary *ext = @{
                            ...
                            notiTextTableValue : @([SharedDataParent date].liveClean),
                            //: NTESSubscribeOnlineState : @(NTESOnlineStateNormal), 
                            user_pointSubMessage : @(NTESOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext json]];
...
- (void)day
{
    ...
    [_subscribeIds addObjectsFromArray:self.maxSection.allObjects];
    //: [_subscribeIds addObjectsFromArray:self.contactUserIds.allObjects];
    [_subscribeIds addObjectsFromArray:self.bit.allObjects];

    //: [self subscribeNextMembers:_subscribeIds.allObjects];
    [self topFrame:_subscribeIds.allObjects];
}

//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)topFrame:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (!isMyFriend && ![self.maxSection containsObject:user.userId]) {
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.bit containsObject:recentSession.session.sessionId]) {
