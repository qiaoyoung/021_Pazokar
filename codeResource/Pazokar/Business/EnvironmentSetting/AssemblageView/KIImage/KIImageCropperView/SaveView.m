...
- (DataWithView *)maskView {
    //: if (_maskView == nil) {
    if (_maskView == nil) {
        //: _maskView = [[KIImageCropperMaskView alloc] initWithFrame:self.bounds];
        _maskView = [[DataWithView alloc] initWithFrame:self.bounds];
...
- (UIImageView *)graftMode {
    //: if (_imageView == nil) {
    if (_imageView == nil) {
        //: _imageView = [[UIImageView alloc] init];
        _imageView = [[UIImageView alloc] init];
        //: [[self scrollView] addSubview:_imageView];
        [[self display] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (void)input:(KIImageCropRatio)ratio {
...
- (void)setImage:(UIImage *)image {
    ...
    if (_image != image) {
        ...
        [[self graftMode] setImage:_image];

        //: [[self imageView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
        [[self graftMode] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
...
- (void)globule {
    ...
    [[self display] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max+5.0f];
    [[self display] setMaximumZoomScale:max+5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self display] setZoomScale:min animated:YES];

    //: [[self scrollView] setContentInset:_imageViewInsets];
    [[self display] setContentInset:_imageViewInsets];
    //: [[self scrollView] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
    [[self display] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    [(DataWithView*)[self maskView] setRefer:[self upwards]];
    //: [[self scrollView] setFrame:self.bounds];
    [[self display] setFrame:self.bounds];
    //: [self updateZoomScale];
    [self globule];
}

//: - (UIImage *)croppedImage {
- (UIImage *)undersideString {
    //: CGFloat zoomScale = [self scrollView].zoomScale;
    CGFloat zoomScale = [self display].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self display].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self display].contentOffset.y;
    ...
    UIImage *image = [_image valueHeight:aX createRecent:aY tapEnable:aWidth time:aHeight];
    ...
    image = [self mentalImage:image blueName:imgSize will:YES];
...
- (UIImage*)mentalImage:(UIImage*)image blueName:(CGSize)newSize will:(BOOL)opaque
{
