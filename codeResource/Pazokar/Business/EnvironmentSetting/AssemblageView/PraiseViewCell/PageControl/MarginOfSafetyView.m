...
- (void)pull
{
    ...
    self.dotViewClass = [NameMoreView class];
    //: self.spacingBetweenDots = kDefaultSpacingBetweenDots;
    self.spacingBetweenDots = appTextTitle;
    //: self.numberOfPages = kDefaultNumberOfPages;
    self.numberOfPages = user_messageSessionIdent;
    //: self.currentPage = kDefaultCurrentPage;
    self.currentPage = mainQuantityName;
    //: self.hidesForSinglePage = kDefaultHideForSinglePage;
    self.hidesForSinglePage = k_trackEventItemValue;
    //: self.shouldResizeFromCenter = kDefaultShouldResizeFromCenter;
    self.shouldResizeFromCenter = show_successTitle;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(progressDown:complete:)]) {
            //: [self.delegate TAPageControl:self didSelectPageAtIndex:index];
            [self.delegate progressDown:self complete:index];
        }
    }
}
...
- (void)sizeToFit
{
    ...
    [self buttonGreen:YES];
}


//: - (CGSize)sizeForNumberOfPages:(NSInteger)pageCount
- (CGSize)ditheredColor:(NSInteger)pageCount
{
...
- (void)communication
{
    ...
    for (NSInteger i = 0; i < self.numberOfPages; i++) {
        ...
        //: } else {
            ...
            dot = [self comeUpRangeView];
        }

        //: [self updateDotFrame:dot atIndex:i];
        [self spectacles:dot from:i];
    }

    //: [self changeActivity:YES atIndex:self.currentPage];
    [self tap:YES will:self.currentPage];
...
- (void)buttonGreen:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self ditheredColor:self.numberOfPages];
...
- (void)spectacles:(UIView *)dot from:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self ditheredColor:self.numberOfPages].width) / 2);
...
- (UIView *)comeUpRangeView
{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[NameMoreView class]] && self.dotColor) {
            //: ((TAAnimatedDotView *)dotView).dotColor = self.dotColor;
            ((NameMoreView *)dotView).dotColor = self.dotColor;
        }
    //: } else {
    } else {
...
- (void)tap:(BOOL)active will:(NSInteger)index
{
    //: if (self.dotViewClass) {
    if (self.dotViewClass) {
        //: TAAbstractDotView *abstractDotView = (TAAbstractDotView *)[self.dots objectAtIndex:index];
        DiscView *abstractDotView = (DiscView *)[self.dots objectAtIndex:index];
        //: if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
        if ([abstractDotView respondsToSelector:@selector(furtherances:)]) {
            //: [abstractDotView changeActivityState:active];
            [abstractDotView furtherances:active];
        //: } else {
        } else {
...
- (void)setCurrentPage:(NSInteger)currentPage
{
    ...
    [self tap:NO will:_currentPage];
    //: _currentPage = currentPage;
    _currentPage = currentPage;
    // Post set
    //: [self changeActivity:YES atIndex:_currentPage];
    [self tap:YES will:_currentPage];
}
