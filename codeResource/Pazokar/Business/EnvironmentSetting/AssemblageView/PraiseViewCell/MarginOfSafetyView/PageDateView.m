...
- (void)contact:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self withPages:self.numberOfPages];
...
- (CGSize)withPages:(NSInteger)pageCount
{
...
- (void)keySelected
{
    ...
    for (NSInteger i = 0; i <= self.numberOfPages; i++) {
        ...
        [self frame:dot dotGreen:i];
    }

    //: [self changeIndicatorViewLayout:self.currentPage];
    [self nameCustom:self.currentPage];
    //: [self hideForSinglePage];
    [self individualityPage];
}
...
- (void)frame:(UIView *)dot dotGreen:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self withPages:self.numberOfPages].width) / 2);
