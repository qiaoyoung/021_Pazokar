...
- (void)layoutSubviews
{
    ...
    if (self.pageViewDelegate && [self.pageViewDelegate respondsToSelector:@selector(requireScrollAnimation)])
    {
        //: animation = [self.pageViewDelegate needScrollAnimation];
        animation = [self.pageViewDelegate requireScrollAnimation];
    }
...
- (void)untilRecent
{
    ...
    if (_scrollView == nil)
    {
        ...
        _scrollView = [[MessageScrollView alloc]initWithFrame:self.bounds];
...
- (NSInteger)parent:(NSInteger)value onDate:(NSInteger)min withMax:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)sessionLine:(NSInteger)currentPage
{
    ...
    NSInteger first = [self parent:currentPage - 1 onDate:0 withMax:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self parent:currentPage + 1 onDate:0 withMax:count - 1];
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        if (NSLocationInRange(index, range))
        {
            ...
            if (![obj isKindOfClass:[UIView class]])
            {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(month:input:)])
                {
                    //: UIView *view = [_dataSource pageView:self viewInPage:index];
                    UIView *view = [_dataSource month:self input:index];
...
- (void)permit
{
    ...
    if(_dataSource && [_dataSource respondsToSelector:@selector(ofses:)])
    {
        //: numberOfPages = [_dataSource numberOfPages:self];
        numberOfPages = [_dataSource ofses:self];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(existed:)])
    {
        //: [_pageViewDelegate pageViewDidScroll:self];
        [_pageViewDelegate existed:self];
    }
}
...
- (void)showDelegate
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(change:convert:faculty:)])
    {
        //: [_pageViewDelegate pageViewScrollEnd:self
        [_pageViewDelegate change:self
                                //: currentIndex:_currentPage
                                convert:_currentPage
                                  //: totolPages:[_pages count]];
                                  faculty:[_pages count]];
    }
}
...
- (BOOL)corner:(UIGestureRecognizer *)gestureRecognizer recognizer:(nonnull UIGestureRecognizer *)otherGestureRecognizer
{
