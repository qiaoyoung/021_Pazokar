...
static CGFloat const userNetworkTitle = 0.1;
//: static CGFloat const MIN_PRESS_TO_BEGIN_EDITING_DURATION = 0.6;
static CGFloat const dream_cellUrl = 0.6;

//: static inline CGPoint CGPointOffset(CGPoint point, CGFloat dx, CGFloat dy)
static inline CGPoint buttonRefreshEnable(CGPoint point, CGFloat dx, CGFloat dy)
{
...
@interface BirthdayFlowLayout () <UIGestureRecognizerDelegate>

//: @property (nonatomic,readonly) id<LxGridViewDataSource> dataSource;
@property (nonatomic,readonly) id<CutMedia> dataSource;
//: @property (nonatomic,readonly) id<LxGridViewDelegateFlowLayout> delegate;
@property (nonatomic,readonly) id<FlowingLayout> delegate;

//: @end
@end

//: @implementation LxGridViewFlowLayout
@implementation BirthdayFlowLayout
{
...
- (void)godspeed
{
    ...
    _longPressGestureRecognizer = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(doQueryionBottom:)];
    //: _longPressGestureRecognizer.cancelsTouchesInView = NO;
    _longPressGestureRecognizer.cancelsTouchesInView = NO;
    //: _longPressGestureRecognizer.minimumPressDuration = PRESS_TO_MOVE_MIN_DURATION;
    _longPressGestureRecognizer.minimumPressDuration = userNetworkTitle;
    ...
    _panGestureRecognizer = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(distances:)];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(streetSmart:) name:UIApplicationWillResignActiveNotification object:nil];
}
...
- (void)doQueryionBottom:(UILongPressGestureRecognizer *)longPress
{
    ...
    switch (longPress.state) {
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (_displayLink == nil) {
                ...
                _displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(publicationing:)];
                ...
                _remainSecondsToBeginEditing = dream_cellUrl;
            }
            ...
            if ([self.delegate respondsToSelector:@selector(outsideHandle:change:textItemPath:)]) {
                //: [self.delegate collectionView:self.collectionView layout:self willBeginDraggingItemAtIndexPath:_movingItemIndexPath];
                [self.delegate outsideHandle:self.collectionView change:self textItemPath:_movingItemIndexPath];
            }
            ...
            TelephoneMessageViewCell *sourceCell = (TelephoneMessageViewCell *)sourceCollectionViewCell;
            ...
            UIView * highlightedSnapshotView = [sourceCell sexMax];
            ...
            UIView * snapshotView = [sourceCell sexMax];
            ...
            [UIView animateWithDuration:0
                             ...
                             completion:^(BOOL finished) {
                                 ...
                                 if (strongSelf) {
                                     ...
                                     if ([strongSelf.delegate respondsToSelector:@selector(nim:buildBy:doCancel:)]) {
                                         ...
                                         [strongSelf.delegate nim:strongSelf.collectionView buildBy:strongSelf doCancel:self->_movingItemIndexPath];
                                     }
                                 }
                             //: }];
                             }];
        ...
        case UIGestureRecognizerStateCancelled:
        {
            ...
            if (movingItemIndexPath) {
                ...
                if ([self.delegate respondsToSelector:@selector(image:scaleFor:cell:)]) {
                    //: [self.delegate collectionView:self.collectionView layout:self willEndDraggingItemAtIndexPath:movingItemIndexPath];
                    [self.delegate image:self.collectionView scaleFor:self cell:movingItemIndexPath];
                }
                ...
                [UIView animateWithDuration:0.2
                                 ...
                                 completion:^(BOOL finished) {
                                     ...
                                     if (strongSelf) {
                                         ...
                                         if ([strongSelf.delegate respondsToSelector:@selector(clubMember:at:byItem:)]) {
                                             ...
                                             [strongSelf.delegate clubMember:strongSelf.collectionView at:strongSelf byItem:movingItemIndexPath];
                                         }
                                     }
                                 //: }];
                                 }];
...
- (void)distances:(UIPanGestureRecognizer *)pan
{
    //: switch (pan.state) {
    switch (pan.state) {
        //: case UIGestureRecognizerStatePossible:
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            _beingMovedPromptView.center = buttonRefreshEnable(_sourceItemCollectionViewCellCenter, panTranslation.x, panTranslation.y);
            ...
            if ([self.dataSource respondsToSelector:@selector(manager:defenceUserWarpath:rangeTo:)] && [self.dataSource manager:self.collectionView defenceUserWarpath:sourceIndexPath rangeTo:destinationIndexPath] == NO) {
                //: return;
                return;
            }

            //: if ([self.dataSource respondsToSelector:@selector(collectionView:itemAtIndexPath:willMoveToIndexPath:)]) {
            if ([self.dataSource respondsToSelector:@selector(atPath:tag:number:)]) {
                ...
                [self.dataSource atPath:self.collectionView tag:sourceIndexPath number:destinationIndexPath];
            }
            ...
            //: } completion:^(BOOL finished) {
                ...
                if ([strongSelf.dataSource respondsToSelector:@selector(sound:color:someItem:)]) {
                    ...
                    [strongSelf.dataSource sound:strongSelf.collectionView color:sourceIndexPath someItem:destinationIndexPath];
                }
            //: }];
            }];
        }
...
- (void)publicationing:(CADisplayLink *)displayLink
{
...
- (void)streetSmart:(NSNotification *)notificaiton
{
