...
@interface FindSessionConfig ()

//: @property (nonatomic,strong) NTESThreadDataSourceProvider *provider;
@property (nonatomic,strong) ValueSourceBubble *provider;
...
@implementation FindSessionConfig
...
- (instancetype)initWithMessageWindow:(NIMMessage *)message
{
    ...
    if (self)
    {
        ...
        _provider = [[ValueSourceBubble alloc] init];
...
@implementation ValueSourceBubble
...
- (void)marginKey:(NIMMessage *)firstMessage scale:(NIMKitDataProvideHandler)handler
{
    ...
    BOOL enablePullCloudMessages = [[EnvironmentSetting configLocation] replacement];
