...
+ (instancetype)old:(NSString *)badgeValue{
    ...
    CenterView *instance = [[CenterView alloc] init];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self border:rect develop:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self keyView:rect immobilize:context];
    }
...
- (CGSize)viewStr:(NSString *)badgeValue{
...
- (CGRect)range:(NSString *)badgeValue{
    ...
    CGSize badgeSize = [self viewStr:badgeValue];
...
- (void)border:(CGRect)rect develop:(CGContextRef)context{
...
- (void)keyView:(CGRect)rect immobilize:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
