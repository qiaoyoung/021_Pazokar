...
static CGFloat const userJoinUrl = 0.75;
//: static BOOL const square = NO;
static BOOL const appMaxCropStr = NO;
//: float IMAGE_MIN_HEIGHT = 400;
float notiMagnitudeKitMessage = 400;
//: float IMAGE_MIN_WIDTH = 400;
float userRangeTitle = 400;
...
#pragma mark IndexViewController implementation
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        self.cropView = [[ImageView alloc] initWithForefront:view bottom:self.blurredBackground];
...
- (void)distants:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(message:title:)])
    {
        ...
        [self.delegate message:self title:cropped];
    }
...
#pragma mark EnableBarView implementation

//: @implementation ControlPointView
@implementation EnableBarView
...
static CGFloat const mInfoUrl = 5;
...
- (void)initTo {
    ...
    self.shadeView = [[LabelView alloc] initWithFrame:subviewFrame];
    ...
    topLeftPoint = [self format:squareLeaveStormCentre(centerInView.x - initialCropAreaSize,
    ...
    bottomLeftPoint = [self format:squareLeaveStormCentre(centerInView.x - initialCropAreaSize,
                                                                      //: centerInView.y + initialCropAreaSize,
                                                                      centerInView.y + initialCropAreaSize,
                                                                      //: controlPointSize)];
                                                                      controlPointSize)];
    ...
    bottomRightPoint = [self format:squareLeaveStormCentre(centerInView.x + initialCropAreaSize,
    ...
    topRightPoint = [self format:squareLeaveStormCentre(centerInView.x + initialCropAreaSize,
                                                                    //: centerInView.y - initialCropAreaSize, controlPointSize)];
                                                                    centerInView.y - initialCropAreaSize, controlPointSize)];

    //the "hole"
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self restrainText];
    ...
    UIPanGestureRecognizer* dragRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(secrets:)];
...
- (EnableBarView*)format:(CGRect)frame {
    //: ControlPointView* point = [[ControlPointView alloc] initWithFrame:frame];
    EnableBarView* point = [[EnableBarView alloc] initWithFrame:frame];
    //: return point;
    return point;
}
...
- (void)locationBase:(CGPoint)topLeft last:(CGPoint)bottomLeft sizeRight:(CGPoint)bottomRight colorPoint:(CGPoint)topRight {
...
- (void)secrets:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan || multiDragPoint.lastCount != count) {
        ...
        if (count > 1)
            ...
            [self countmate:recognizer pin:count];
        //: else
        else
            //: [self prepSingleTouchPan:recognizer];
            [self languageBy:recognizer];
    ...
    if (count > 1) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            [self frame:[recognizer locationOfTouch:i inView:self] size:(i == 0 ? dragViewOne : dragViewTwo)];
    ...
    //: } else {
        ...
        [self frame:[recognizer locationInView:self] size:dragViewOne];
    }
...
- (void)countmate:(UIPanGestureRecognizer*)recognizer pin:(NSUInteger)count
{
    //: for (int i = 0; i < count; i++) {
    for (int i = 0; i < count; i++) {
        //: if (i == 0) {
        if (i == 0) {
            ...
            dragViewOne = [self to:[recognizer locationOfTouch:i inView:self]];
        ...
        //: } else {
            ...
            dragViewTwo = [self to:[recognizer locationOfTouch:i inView:self]];
...
- (void)languageBy:(UIPanGestureRecognizer*)recognizer
{
    //: dragViewOne = [self checkHit:[recognizer locationInView:self]];
    dragViewOne = [self to:[recognizer locationInView:self]];
...
- (void)frame:(CGPoint)location size:(UIView*)view
{
    //: if (view == topLeftPoint) {
    if (view == topLeftPoint) {
        //: [self handleDragTopLeft:location];
        [self handleSessionLeftValue:location];
...
- (CGSize)key:(CGPoint)dragLocation middle:(CGPoint)draggedPoint all:(CGPoint)oppositePoint {
...
- (void)handleSessionLeftValue:(CGPoint)dragLocation {
    ...
    CGSize disp = [self key:dragLocation middle:dragPoint.topLeftCenter all:dragPoint.bottomRightCenter];
    ...
    if (width >= userRangeTitle && height < notiMagnitudeKitMessage) {
        //: topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - IMAGE_MIN_HEIGHT) / self.imageScale);
        topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - notiMagnitudeKitMessage) / self.imageScale);
        //: topRight.y = topLeft.y;
        topRight.y = topLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < userRangeTitle && height >= notiMagnitudeKitMessage) {
        //: topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - userRangeTitle) / self.imageScale);
    ...
    [self locationBase:topLeft last:bottomLeft sizeRight:dragPoint.bottomRightCenter colorPoint:topRight];
}
...
- (void)path:(CGPoint)dragLocation {
    ...
    CGSize disp = [self key:dragLocation middle:dragPoint.bottomLeftCenter all:dragPoint.topRightCenter];
    ...
    if (width >= userRangeTitle && height < notiMagnitudeKitMessage) {
        ...
        bottomLeft.y = cropArea.origin.y + (notiMagnitudeKitMessage / self.imageScale);
        //: bottomRight.y = bottomLeft.y;
        bottomRight.y = bottomLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < userRangeTitle && height >= notiMagnitudeKitMessage) {
        //: bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - userRangeTitle) / self.imageScale);
    ...
    [self locationBase:topLeft last:bottomLeft sizeRight:bottomRight colorPoint:dragPoint.topRightCenter];
}

//: - (void)handleDragBottomRight:(CGPoint)dragLocation {
- (void)overDate:(CGPoint)dragLocation {
    ...
    CGSize disp = [self key:dragLocation middle:dragPoint.bottomRightCenter all:dragPoint.topLeftCenter];
    ...
    if (width >= userRangeTitle && height < notiMagnitudeKitMessage) {
        ...
        bottomRight.y = cropArea.origin.y + (notiMagnitudeKitMessage / self.imageScale);
        //: bottomLeft.y = bottomRight.y;
        bottomLeft.y = bottomRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < userRangeTitle && height >= notiMagnitudeKitMessage) {
        //: bottomRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        bottomRight.x = cropArea.origin.x + (userRangeTitle / self.imageScale);
    ...
    [self locationBase:dragPoint.topLeftCenter last:bottomLeft sizeRight:bottomRight colorPoint:topRight];
}

//: - (void)handleDragTopRight:(CGPoint)dragLocation {
- (void)endReadSend:(CGPoint)dragLocation {
    ...
    CGSize disp = [self key:dragLocation middle:dragPoint.topRightCenter all:dragPoint.bottomLeftCenter];
    ...
    if (width >= userRangeTitle && height < notiMagnitudeKitMessage) {
        ...
        topRight.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - notiMagnitudeKitMessage) / self.imageScale);
        //: topLeft.y = topRight.y;
        topLeft.y = topRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < userRangeTitle && height >= notiMagnitudeKitMessage) {
        //: topRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        topRight.x = cropArea.origin.x + (userRangeTitle / self.imageScale);
    ...
    [self locationBase:topLeft last:dragPoint.bottomLeftCenter sizeRight:bottomRight colorPoint:topRight];
}

//: - (void)handleDragCropArea:(CGPoint)dragLocation {
- (void)large:(CGPoint)dragLocation {
    //: CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    //: CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
    CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
...
- (void)setImage:(UIImage *)image {
    ...
    if (self.blurred) {
        ...
        blur = [image bubble:30 magnitudeernalRepresentation:1 uncolored:[UIColor blackColor]];
    //: } else {
    } else {
        //: blur = [image blurredImageWithRadius:0 iterations:1 tintColor:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
        blur = [image bubble:0 magnitudeernalRepresentation:1 uncolored:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
    }
    ...
    [self locationBase:topLeftPoint.center last:bottomLeftPoint.center sizeRight:bottomRightPoint.center colorPoint:topRightPoint.center];
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self restrainText];
    //: cropAreaView.frame = cropArea;
    cropAreaView.frame = cropArea;
...
@implementation UIImage (Ameliorate)
