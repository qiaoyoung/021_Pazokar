...
- (Byte *)HeadDataToCache:(Byte *)data {
    int guitarInfo = data[0];
    Byte attemptTower = data[1];
    int semenDemonIdeal = data[2];
    for (int i = semenDemonIdeal; i < semenDemonIdeal + guitarInfo; i++) {
        int value = data[i] - attemptTower;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[semenDemonIdeal + guitarInfo] = 0;
    return data + semenDemonIdeal;
}
...
//: http
- (NSString *)app_negativeValue {
    /* static */ NSString *app_negativeValue = nil;
    if (!app_negativeValue) {
		NSString *origin = @"04450AF638CCDF88B183ADB9B9B567";
		NSData *data = [HeadData HeadDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        app_negativeValue = [self StringFromHeadData:value];
    }
    return app_negativeValue;
}
...
NSString * const mContentText = @"cycleCell";

//: @interface SDCycleScrollView () <UICollectionViewDataSource, UICollectionViewDelegate>
@interface BackgroundView () <UICollectionViewDataSource, UICollectionViewDelegate>
...
+ (instancetype)pastGroup:(CGRect)frame searchedTo:(NSArray *)imageNamesGroup
{
    //: SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    BackgroundView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)image:(CGRect)frame send:(BOOL)infiniteLoop data:(NSArray *)imageNamesGroup
{
    //: SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    BackgroundView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)add:(CGRect)frame cell:(NSArray *)imageURLsGroup
{
    //: SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    BackgroundView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)enable:(CGRect)frame mentalRepresentation:(id<SoundDelegate>)delegate model:(UIImage *)placeholderImage
{
    //: SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    BackgroundView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
- (void)sizeMessage
{
    ...
    [mainView registerClass:[PraiseViewCell class] forCellWithReuseIdentifier:mContentText];
...
- (void)setPageControlDotSize:(CGSize)pageControlDotSize
{
    ...
    if ([self.pageControl isKindOfClass:[MarginOfSafetyView class]]) {
        //: TAPageControl *pageContol = (TAPageControl *)_pageControl;
        MarginOfSafetyView *pageContol = (MarginOfSafetyView *)_pageControl;
...
- (void)setCurrentPageDotColor:(UIColor *)currentPageDotColor
{
    ...
    if ([self.pageControl isKindOfClass:[MarginOfSafetyView class]]) {
        ...
        MarginOfSafetyView *pageControl = (MarginOfSafetyView *)_pageControl;
...
- (void)setCurrentPageDotImage:(UIImage *)currentPageDotImage
{
    ...
    [self than:currentPageDotImage color:YES];
}
...
- (void)setPageDotImage:(UIImage *)pageDotImage
{
    ...
    [self than:pageDotImage color:NO];
}

//: - (void)setCustomPageControlDotImage:(UIImage *)image isCurrentPageDot:(BOOL)isCurrentPageDot
- (void)than:(UIImage *)image color:(BOOL)isCurrentPageDot
{
...
- (void)provider
{
    ...
    NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:self.autoScrollTimeInterval target:self selector:@selector(subWith) userInfo:nil repeats:YES];
    //: _timer = timer;
    _timer = timer;
...
- (void)sound
{
    ...
    int indexOnPageControl = [self afootIndex] % self.imagePathsGroup.count;
    ...
    switch (self.pageControlStyle) {
        ...
        case SDCycleScrollViewPageContolStyleStrip:
        {
            ...
            PageDateView *pageControl = [[PageDateView alloc] init];
        ...
        case SDCycleScrollViewPageContolStyleAnimated:
        {
            ...
            MarginOfSafetyView *pageControl = [[MarginOfSafetyView alloc] init];
            //: pageControl.numberOfPages = self.imagePathsGroup.count;
            pageControl.numberOfPages = self.imagePathsGroup.count;
            //: pageControl.dotColor = self.currentPageDotColor;
            pageControl.dotColor = self.currentPageDotColor;
...
- (void)subWith
{
    ...
    int currentIndex = [self afootIndex];
...
- (void)layoutSubviews
{
    ...
    if ([self.pageControl isKindOfClass:[MarginOfSafetyView class]]) {
        ...
        size = [pageControl ditheredColor:self.imagePathsGroup.count];
    }
    //: else if ([self.pageControl isKindOfClass:[TTStripPageControl class]]){
    else if ([self.pageControl isKindOfClass:[PageDateView class]]){
        //: TTStripPageControl *pageControl = (TTStripPageControl *)_pageControl;
        PageDateView *pageControl = (PageDateView *)_pageControl;
        //: size = [pageControl sizeForNumberOfPages:self.imagePathsGroup.count];
        size = [pageControl withPages:self.imagePathsGroup.count];
    }
    //: else {
    else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PraiseViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:mContentText forIndexPath:indexPath];
    ...
    if ([imagePath isKindOfClass:[NSString class]]) {
        ...
        if ([imagePath hasPrefix:[[HeadData sharedInstance] app_negativeValue]]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(query_strong:key:)]) {
        //: [self.delegate cycleScrollView:self didSelectItemAtIndex:indexPath.item % self.imagePathsGroup.count];
        [self.delegate query_strong:self key:indexPath.item % self.imagePathsGroup.count];
    }
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    if ([self.delegate respondsToSelector:@selector(statusEdge:showLanguage:)]) {
        //: [self.delegate cycleScrollView:self didScrollToIndex:indexOnPageControl];
        [self.delegate statusEdge:self showLanguage:indexOnPageControl];
