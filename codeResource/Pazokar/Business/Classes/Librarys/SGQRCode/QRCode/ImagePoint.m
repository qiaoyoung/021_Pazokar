...
@interface ImagePoint () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
{
    //: SGSoundEffect *soundEffect;
    CanEffect *soundEffect;
}
...
- (void)setDelegate:(id<ViewModel>)delegate {
    //: _delegate = delegate;
    _delegate = delegate;
...
- (void)setSampleBufferDelegate:(id<MarginGlassFormat>)sampleBufferDelegate {
...
- (void)button:(UIImage *)image from:(void (^)(NSString *result))completion {
...
- (void)setRandomSuccess:(CGFloat)factor {
...
- (void)scalePhenomenon:(NSString *)name {
    ...
    NSString *voicePath = [[[ValuePraise show] getWithPath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    ...
    soundEffect = [CanEffect bearing:voicePath];
    //: [soundEffect play];
    [soundEffect progressPlay];
}
...
- (AVCaptureSession *)session {
    ...
    if (!_session) {
        ...
        _session.sessionPreset = [self send];
    }
    //: return _session;
    return _session;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(back:pushResult:)]) {
                //: [self.delegate scanCode:self result:resultString];
                [self.delegate back:self pushResult:resultString];
            }
        //: });
        });
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.sampleBufferDelegate && [self.sampleBufferDelegate respondsToSelector:@selector(close:sittingOf:)]) {
            //: [self.sampleBufferDelegate scanCode:self brightness:brightnessValue];
            [self.sampleBufferDelegate close:self sittingOf:brightnessValue];
        }
    //: });
    });
}
