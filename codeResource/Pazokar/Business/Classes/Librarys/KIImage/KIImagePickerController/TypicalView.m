...
#import "TypicalView.h"

//: @interface KICropImageMaskView : UIView {
@interface CropTingForefrontView : UIView {
//: @private
@private
    //: CGRect _cropRect;
    CGRect _cropRect;
...
- (void)setAllowValue:(CGSize)size;
//: - (CGSize)cropSize;
- (CGSize)allowValue;
//: @end
@end
...
- (void)setFrame:(CGRect)frame {
    ...
    [[self viewScroll] setFrame:self.bounds];
    ...
    if (__CGSizeEqualToSize(_cropSize, CGSizeZero)) {
        ...
        [self setInscription:CGSizeMake(100, 100)];
    }
}
...
- (UIImageView *)deal {
    ...
    if (_imageView == nil) {
        ...
        [[self viewScroll] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (CropTingForefrontView *)maskView {
    //: if (_maskView == nil) {
    if (_maskView == nil) {
        //: _maskView = [[KICropImageMaskView alloc] init];
        _maskView = [[CropTingForefrontView alloc] init];
...
- (void)sessionScale {
    ...
    [[self deal] setFrame:CGRectMake(0, 0, width, height)];
...
//    }
    ...
    [[self viewScroll] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max + 5.0f];
    [[self viewScroll] setMaximumZoomScale:max + 5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self viewScroll] setZoomScale:min animated:YES];
}
...
- (void)setInscription:(CGSize)size {
    ...
    [(CropTingForefrontView *)[self maskView] setAllowValue:_cropSize];
    ...
    [[self viewScroll] setContentInset:_imageInset];
...
- (UIImage *)can {
    ...
    CGFloat zoomScale = [self viewScroll].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self viewScroll].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self viewScroll].contentOffset.y;
    ...
    image = [image tip:resizeWidth disable:resizeheight];
...
@implementation CropTingForefrontView
