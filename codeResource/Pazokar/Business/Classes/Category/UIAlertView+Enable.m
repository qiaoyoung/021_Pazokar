...
static char dream_portraitFormat;

//: @implementation UIAlertView (NTESBlock)
@implementation UIAlertView (Enable)
//: - (void)showAlertWithCompletionHandler: (void (^)(NSInteger))block
- (void)alertHandler: (void (^)(NSInteger))block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIAlertViewBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&dream_portraitFormat,block,OBJC_ASSOCIATION_COPY);
    //: [self show];
    [self show];
}
...
- (void)white:(UIAlertView *)alertView input:(NSInteger)buttonIndex
{
    //: AlertBlock block = objc_getAssociatedObject(self, &kUIAlertViewBlockAddress);
    AlertBlock block = objc_getAssociatedObject(self, &dream_portraitFormat);
    //: if (block)
    if (block)
    {
        //: block(buttonIndex);
        ...
        objc_setAssociatedObject(self, &dream_portraitFormat, nil, OBJC_ASSOCIATION_COPY);
    }
}
...
@implementation UIAlertController (Enable)
