 ...
 *************************简要说明************************
 ...
 [StraddleAlert configMainWindow:self.window];
 
 
 Player 使用方法
 
 [StraddleAlert alert].cofing.XXXXX.XXXXX.LeeShow();
 
 Accenting 使用方法
 
 [StraddleAlert actionSheet].cofing.XXXXX.XXXXX.LeeShow();
...
@interface StraddleAlert: NSObject

/** 初始化 */

//: + (nonnull LEEAlertConfig *)alert;
+ (nonnull VersionBaseConfig *)view;

//: + (nonnull LEEActionSheetConfig *)actionsheet;
+ (nonnull CustomBaseConfig *)close;

/** 获取Alert窗口 */
//: + (nonnull LEEAlertWindow *)getAlertWindow;
+ (nonnull RadiusButtonDormerView *)session;
...
+ (void)to:(NSString *)identifier ambientWithinBlock:(BOOL)force identifierBlock:(void (^ _Nullable)(void))completionBlock;
...
+ (void)cubeBlock:(void (^ _Nullable)(void))completionBlock;

//: @end
@end

//: @interface LEEBaseConfigModel: NSObject
@interface MentalImageModel: NSObject
...
/** 设置 添加自定义视图 -> 格式: .LeeAddCustomView(^(BackgroundDirection *){ //code.. }) */
//: @property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;
@property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;

/** 设置 添加一项 -> 格式: .LeeAddItem(^(ContentEmo *){ //code.. }) */
//: @property (nonatomic, copy, readonly) LEEConfigToItem LeeAddItem;
@property (nonatomic, copy, readonly) LEEConfigToItem LeeAddItem;

/** 设置 添加动作 -> 格式: .LeeAddAction(^(FormatRedApp *){ //code.. }) */
...
@interface MentalImageModel(Presentation)

/** 设置 层级 -> 格式: .LeePresentation([DataMessage windowLevel:1000] / [DataMessage viewController:xxxx]) */
...
@interface MentalImageModel(Queue)
...
@interface MentalImageModel(Layout)
...
@interface MentalImageModel(Animation)
...
@interface MentalImageModel(Callback)
...
@interface MentalImageModel(Player)
...
@interface MentalImageModel(Accenting)
...
@interface FormatRedApp: NSObject
...
@interface BackgroundDirection: NSObject
...
+ (DataMessage *)selected: (UIWindowLevel)level;

//: + (LEEPresentation *)viewController: (UIViewController *)controller;
+ (DataMessage *)beBy: (UIViewController *)controller;
...
@property (nonatomic, strong, nonnull) MentalImageModel *config;

//: @end
@end

//: @interface LEEAlertConfig: LEEBaseConfig
@interface VersionBaseConfig: TitleApp

//: @end
@end

//: @interface LEEActionSheetConfig: LEEBaseConfig
@interface CustomBaseConfig: TitleApp

//: @end
@end

//: @interface LEEAlertWindow: UIWindow @end
@interface RadiusButtonDormerView: UIWindow @end

//: @interface LEEBaseViewController: UIViewController @end
@interface DenominateViewController: UIViewController @end

//: @interface LEEAlertViewController: LEEBaseViewController @end
@interface EigenvalueOfASquareMatrixViewController: DenominateViewController @end

//: @interface LEEActionSheetViewController: LEEBaseViewController @end
@interface MaturityDateViewController: DenominateViewController @end
...
Background textFinish(void);

//: CornerRadii CornerRadiiNull(void);
Background areaPull(void);
