...
@interface BoarTopicData : NSObject

@end

@implementation BoarTopicData

+ (NSData *)BoarTopicDataToData:(NSString *)value {
...
+ (Byte *)BoarTopicDataToCache:(Byte *)data {
    int separateShow = data[0];
    Byte nameTower = data[1];
    int reliability = data[2];
    for (int i = reliability; i < reliability + separateShow; i++) {
        int value = data[i] - nameTower;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[reliability + separateShow] = 0;
    return data + reliability;
}

+ (NSString *)StringFromBoarTopicData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self BoarTopicDataToCache:data]];
}

//: #02D8C9
+ (NSString *)userWithKey {
    /* static */ NSString *userWithKey = nil;
    if (!userWithKey) {
		NSString *origin = @"0706032936384a3e493f7d";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        userWithKey = [self StringFromBoarTopicData:value];
    }
    return userWithKey;
}

//: select_contact
+ (NSString *)showValueImageMsg {
    /* static */ NSString *showValueImageMsg = nil;
    if (!showValueImageMsg) {
		NSString *origin = @"0e4f04a1c2b4bbb4b2c3aeb2bebdc3b0b2c353";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        showValueImageMsg = [self StringFromBoarTopicData:value];
    }
    return showValueImageMsg;
}

//: icon_checkbox_selected
+ (NSString *)mainRisingTouristMsg {
    /* static */ NSString *mainRisingTouristMsg = nil;
    if (!mainRisingTouristMsg) {
		NSString *origin = @"162209518fc14ce5428b85919081858a87858d84919a8195878e8785968786b2";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        mainRisingTouristMsg = [self StringFromBoarTopicData:value];
    }
    return mainRisingTouristMsg;
}

//: SelectContactCellID
+ (NSString *)notiBarData {
    /* static */ NSString *notiBarData = nil;
    if (!notiBarData) {
		NSString *origin = @"13190bd14e13f9012676c06c7e857e7c8d5c88878d7a7c8d5c7e8585625dea";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        notiBarData = [self StringFromBoarTopicData:value];
    }
    return notiBarData;
}

//: ic_choose_w
+ (NSString *)userRegardingTitle {
    /* static */ NSString *userRegardingTitle = nil;
    if (!userRegardingTitle) {
		NSString *origin = @"0b14049e7d7773777c83838779738b5f";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        userRegardingTitle = [self StringFromBoarTopicData:value];
    }
    return userRegardingTitle;
}

//: #999999
+ (NSString *)appSeniorKey {
    /* static */ NSString *appSeniorKey = nil;
    if (!appSeniorKey) {
		NSString *origin = @"075c0d60b235711b4970eb96be7f95959595959555";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        appSeniorKey = [self StringFromBoarTopicData:value];
    }
    return appSeniorKey;
}

//: No friends，please add friends
+ (NSString *)appTightenValue {
    /* static */ NSString *appTightenValue = nil;
    if (!appTightenValue) {
		NSString *origin = @"1f3409fb714a05bfd782a3549aa69d99a298a723f0c0a4a09995a79954959898549aa69d99a298a77e";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        appTightenValue = [self StringFromBoarTopicData:value];
    }
    return appTightenValue;
}

//: main_activity_choose_least_one
+ (NSString *)notiSeedNameData {
    /* static */ NSString *notiSeedNameData = nil;
    if (!notiSeedNameData) {
		NSString *origin = @"1e050c1b74e4c3996edf123672666e73646668796e7b6e797e64686d7474786a64716a6678796474736a97";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        notiSeedNameData = [self StringFromBoarTopicData:value];
    }
    return notiSeedNameData;
}

//: no_friend
+ (NSString *)m_backgroundStr {
    /* static */ NSString *m_backgroundStr = nil;
    if (!m_backgroundStr) {
		NSString *origin = @"0937095ab8987e1132a5a6969da9a09ca59bd0";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        m_backgroundStr = [self StringFromBoarTopicData:value];
    }
    return m_backgroundStr;
}

//: ic_close_b
+ (NSString *)k_sliceIdent {
    /* static */ NSString *k_sliceIdent = nil;
    if (!k_sliceIdent) {
		NSString *origin = @"0a58053ff4c1bbb7bbc4c7cbbdb7baa0";
		NSData *data = [BoarTopicData BoarTopicDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        k_sliceIdent = [self StringFromBoarTopicData:value];
    }
    return k_sliceIdent;
}
...
- (void)viewDidLoad
{
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, (44.0f + [UIDevice statusOrLevel]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice statusOrLevel])) style:UITableViewStylePlain];
...
- (void)buttonPostpose
{
    ...
    [backButton setImage:[UIImage imageNamed:[BoarTopicData k_sliceIdent]] forState:(UIControlStateNormal)];
    ...
    submitButton.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-15-48, [UIDevice statusOrLevel]+10, 48, 28);
    //: submitButton.backgroundColor = [UIColor colorWithHexString:@"#02D8C9"];
    submitButton.backgroundColor = [UIColor status:[BoarTopicData userWithKey]];
    //: submitButton.layer.cornerRadius = 14;
    submitButton.layer.cornerRadius = 14;
    //: [submitButton setImage:[UIImage imageNamed:@"ic_choose_w"] forState:(UIControlStateNormal)];
    [submitButton setImage:[UIImage imageNamed:[BoarTopicData userRegardingTitle]] forState:(UIControlStateNormal)];
    //: [submitButton addTarget:self action:@selector(onDoneBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [submitButton addTarget:self action:@selector(sessionVersion:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)mediaWith
{
    ...
    if(_selectecContacts.count > 0){
        ...
        [btn setTitle:self.hide forState:UIControlStateNormal];
        //: [btn setImage:[UIImage imageNamed:@"icon_checkbox_selected"] forState:UIControlStateNormal];
        [btn setImage:[UIImage imageNamed:[BoarTopicData mainRisingTouristMsg]] forState:UIControlStateNormal];
...
- (void)inscription
{
    ...
    if(_selectecContacts.count > 0){
        ...
        self.tableView.frame = CGRectMake(0, (44.0f + [UIDevice statusOrLevel])+64, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice statusOrLevel])-64);
    //: }else{
    }else{
        ...
        self.tableView.frame = CGRectMake(0, (44.0f + [UIDevice statusOrLevel]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice statusOrLevel]));
    }
}
...
- (void)setConfig:(id<NameConfig>)config{
    ...
    if ([config respondsToSelector:@selector(searchedTag)]) {
        //: _maxSelectCount = [config maxSelectedNum];
        _maxSelectCount = [config searchedTag];
...
- (void)doingInfoRemote:(id)sender {
    ...
    [self dismissViewControllerAnimated:YES completion:^() {
        ...
        if([_delegate respondsToSelector:@selector(bubbleBottom)]) {
            //: [_delegate didCancelledSelect];
            [_delegate bubbleBottom];
        }
    //: }];
    }];
}
...
- (IBAction)sessionVersion:(id)sender {
    ...
    if (self.config.showSelectHeaderview) {
        ...
        [self.setGroupnameView threadMessageShow];
        @
            ...
            @
             ...
             try{} @finally{} __typeof__(self) self = __weak_self__;
                ...
                if (_selectecContacts.count) {
                    ...
                    if ([self.delegate respondsToSelector:@selector(fillInSearch:)]) {
...
- (void)clean{
    ...
    [self.config imageData:^(NSDictionary *contentDic, NSArray *titles) {
    ...
    if ([self.config respondsToSelector:@selector(labelWith)])
    {
        //: _selectecContacts = [[self.config alreadySelectedMemberId] mutableCopy];
        _selectecContacts = [[self.config labelWith] mutableCopy];
    }
    ...
    for (NSString *selectId in _selectecContacts) {
        ...
        [self.selectIndicatorView.pickedView distanceNim:info];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.accessoryBtn.selected = [_selectecContacts containsObject:[contactItem conference]];
    //: [cell refreshItem:contactItem];
    [cell selectChangeTab:contactItem];
    //: return cell;
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    id<DatePraise> member = arr[indexPath.row];
...
- (UIView *)defView{
    ...
    if(!_defView){
        ...
        _defView = [[UIView alloc]initWithFrame:CGRectMake(0, (44.0f + [UIDevice statusOrLevel]), [[UIScreen mainScreen] bounds].size.width, 400)];
        //: _defView.hidden = YES;
        _defView.hidden = YES;
        ...
        defImg.image = [UIImage imageNamed:[BoarTopicData m_backgroundStr]];
        ...
        emptyTipLabel.textColor = [UIColor status:[BoarTopicData appSeniorKey]];
        ...
        emptyTipLabel.text = [BoarTopicData appTightenValue];
