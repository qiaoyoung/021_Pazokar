...
@interface DeserveData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation DeserveData

+ (instancetype)sharedInstance {
    static DeserveData *instance = nil;
...
+ (NSData *)DeserveDataToData:(NSString *)value {
...
- (Byte *)DeserveDataToCache:(Byte *)data {
    int scapeReplyCede = data[0];
    Byte scapeAye = data[1];
    int upTo = data[2];
    for (int i = upTo; i < upTo + scapeReplyCede; i++) {
        int value = data[i] + scapeAye;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[upTo + scapeReplyCede] = 0;
    return data + upTo;
}

- (NSString *)StringFromDeserveData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self DeserveDataToCache:data]];
}

//: emoji_del_normal
- (NSString *)kCargoUrl {
    /* static */ NSString *kCargoUrl = nil;
    if (!kCargoUrl) {
		NSString *origin = @"1063043f020a0c0706fc010209fc0b0c0f0afe099f";
		NSData *data = [DeserveData DeserveDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        kCargoUrl = [self StringFromDeserveData:value];
    }
    return kCargoUrl;
}

//: emoji_del_pressed
- (NSString *)k_sizeValueText {
    /* static */ NSString *k_sizeValueText = nil;
    if (!k_sizeValueText) {
		NSString *origin = @"112e0cdd29bcc0f67d5d36b3373f413c3b3136373e31424437454537364f";
		NSData *data = [DeserveData DeserveDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        k_sizeValueText = [self StringFromDeserveData:value];
    }
    return k_sizeValueText;
}

//: gif
- (NSString *)dreamVirusMessage {
    /* static */ NSString *dreamVirusMessage = nil;
    if (!dreamVirusMessage) {
		NSString *origin = @"03370ab40ad1256a4af430322f18";
		NSData *data = [DeserveData DeserveDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        dreamVirusMessage = [self StringFromDeserveData:value];
    }
    return dreamVirusMessage;
}
...
- (NSString *)main_successText {
    ...
    if (!main_successText) {
		NSString *origin = @"0758073d725c98cbeedeeedfeee93f";
		NSData *data = [DeserveData DeserveDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        main_successText = [self StringFromDeserveData:value];
    }
    return main_successText;
}

//: default
- (NSString *)dream_ayeMessage {
    /* static */ NSString *dream_ayeMessage = nil;
    if (!dream_ayeMessage) {
		NSString *origin = @"072d045f37383934483f4790";
		NSData *data = [DeserveData DeserveDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        dream_ayeMessage = [self StringFromDeserveData:value];
    }
    return dream_ayeMessage;
}
...
NSInteger show_styleStr = 36;
//: NSInteger NIMCustomPageViewHeight = 159;
NSInteger notiNamePath = 159;

//: @interface BusyInputEmoticonContainerView()<NIMEmoticonButtonTouchDelegate,NIMInputEmoticonTabDelegate>
@interface EastWestDirectionView()<IndexChild,BirthdayDelegate>

//: @property (nonatomic,strong) NSMutableArray *pageData;
@property (nonatomic,strong) NSMutableArray *pageData;

//: @property (nonatomic,strong) BusyInputEmoticonButton* deleteIcon;
@property (nonatomic,strong) PointButton* deleteIcon;

//: @end
@end


//: @implementation BusyInputEmoticonContainerView
@implementation EastWestDirectionView
...
//    self.backgroundColor = [UIColor clearColor];
    ...
    self.backgroundColor = [UIColor status:[[DeserveData sharedInstance] main_successText]];
}
...
//    [_emotPageController setUserInteractionEnabled:NO];
    ...
    CGFloat heght = (self.currentCatalogData.emoticons.count/self.currentCatalogData.layout.columes +1)*46.0 +show_styleStr;
    //: [self.scrollView setContentSize:CGSizeMake(self.nim_width, heght )];
    [self.scrollView setContentSize:CGSizeMake(self.nim_width, heght )];
//
//    _deleteIcon = [[PointButton alloc] init];
...
- (void)object{
    ...
    NSArray *data = [self catalogueLightAndStack];
...
- (NSArray *)catalogueLightAndStack
{
    //: NIMInputEmoticonCatalog *defaultCatalog = [self loadDefaultCatalog];
    ImageColor *defaultCatalog = [self blue];
...
//    NSArray *catalogs = defaultCatalog? [@[defaultCatalog] arrayByAddingObjectsFromArray:charlets] : charlets;
        ...
        ImageColor *gifCatalog = [self buttonCreate];
...
- (void)layoutBubble
{
    ...
    if(self.currentCatalogData.emoticons.count>0){
        ...
        for (NSInteger index = 0; index < self.currentCatalogData.emoticons.count; index ++)
        {
            ...
            CityCenterWith *data = [self.currentCatalogData.emoticons objectAtIndex:index];

            //: BusyInputEmoticonButton *button = [BusyInputEmoticonButton iconButtonWithData:data catalogID:self.currentCatalogData.catalogID delegate:self];
            PointButton *button = [PointButton input:data holdfast:self.currentCatalogData.catalogID icon:self];
            //计算表情位置
...
- (NSInteger)build
{
    ...
    [self.totalCatalogData enumerateObjectsUsingBlock:^(ImageColor* data, NSUInteger idx, BOOL *stop) {
...
//: - (UIView*)emojPageView:(BusyPageView*)pageView inEmoticonCatalog:(NIMInputEmoticonCatalog *)emoticon page:(NSInteger)page
- (UIView*)inLayer:(PageView*)pageView emojLabelPage:(ImageColor *)emoticon icon:(NSInteger)page
{
    ...
    for (NSInteger index = begin; index < end; index ++)
    {
        ...
        CityCenterWith *data = [emoticon.emoticons objectAtIndex:index];

        //: BusyInputEmoticonButton *button = [BusyInputEmoticonButton iconButtonWithData:data catalogID:emoticon.catalogID delegate:self];
        PointButton *button = [PointButton input:data holdfast:emoticon.catalogID icon:self];
        //计算表情位置
...
- (void)canTableColor:(UIView *)view
                      //: ColumnIndex:(NSInteger)coloumnIndex
                      hide:(NSInteger)coloumnIndex
                         //: RowIndex:(NSInteger)rowIndex
                         cell:(NSInteger)rowIndex
                           //: StartX:(CGFloat)startX
                           house:(CGFloat)startX
                           //: StartY:(CGFloat)startY
                           randomOf:(CGFloat)startY
                        //: IconWidth:(CGFloat)iconWidth
                        tabletUser:(CGFloat)iconWidth
                       //: IconHeight:(CGFloat)iconHeight
                       saveCatalog:(CGFloat)iconHeight
                //: inEmoticonCatalog:(NIMInputEmoticonCatalog *)emoticon
                canByImage:(ImageColor *)emoticon
{
    //: BusyInputEmoticonButton* deleteIcon = [[BusyInputEmoticonButton alloc] init];
    PointButton* deleteIcon = [[PointButton alloc] init];
    ...
    deleteIcon.contentMode = UIViewContentModeCenter;
    //: UIImage *imageNormal = [UIImage nim_emoticonInKit:@"emoji_del_normal"];
    UIImage *imageNormal = [UIImage quickKit:[[DeserveData sharedInstance] kCargoUrl]];
    //: UIImage *imagePressed = [UIImage nim_emoticonInKit:@"emoji_del_pressed"];
    UIImage *imagePressed = [UIImage quickKit:[[DeserveData sharedInstance] k_sizeValueText]];
    ...
    [deleteIcon addTarget:deleteIcon action:@selector(addStorage:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIView *)month:(PageView *)pageView input:(NSInteger)index
{
    ...
    return [self inLayer:pageView emojLabelPage:emoticon icon:index - page];
}
...
- (ImageColor*)blue
{
    //: NIMInputEmoticonCatalog *emoticonCatalog = [[BusyInputEmoticonManager sharedManager] emoticonCatalog:@"default"];
    ImageColor *emoticonCatalog = [[DeviceManager history] play:[[DeserveData sharedInstance] dream_ayeMessage]];
    //: if (emoticonCatalog) {
    if (emoticonCatalog) {
        //: NIMInputEmoticonLayout *layout = [[NIMInputEmoticonLayout alloc] initEmojiLayout:self.nim_width];
        SignalingLayout *layout = [[SignalingLayout alloc] initShowTo:self.nim_width];
        //: emoticonCatalog.layout = layout;
        emoticonCatalog.layout = layout;
        //: emoticonCatalog.pagesCount = [self numberOfPagesWithEmoticon:emoticonCatalog];
        emoticonCatalog.pagesCount = [self tool:emoticonCatalog];
    }
...
- (ImageColor*)buttonCreate
{
    //: NIMInputEmoticonCatalog *emoticonCatalog = [[BusyInputEmoticonManager sharedManager] emoticonCatalog:@"gif"];
    ImageColor *emoticonCatalog = [[DeviceManager history] play:[[DeserveData sharedInstance] dreamVirusMessage]];
    //: if (emoticonCatalog) {
    if (emoticonCatalog) {
        //: NIMInputEmoticonLayout *layout = [[NIMInputEmoticonLayout alloc] initEmojiLayout:self.nim_width];
        SignalingLayout *layout = [[SignalingLayout alloc] initShowTo:self.nim_width];
...
- (NSInteger)content:(ImageColor *)emoticonCatalog{
    //: NSInteger pageIndex = 0;
    NSInteger pageIndex = 0;
    //: for (NIMInputEmoticonCatalog *emoticon in self.totalCatalogData) {
    for (ImageColor *emoticon in self.totalCatalogData) {
...
- (NSInteger)holder:(NSInteger)index{
    //: NIMInputEmoticonCatalog *catelog = [self emoticonWithIndex:index];
    ImageColor *catelog = [self streetSmartIndexVideo:index];
    //: NSInteger begin = [self pageIndexWithEmoticon:catelog];
    NSInteger begin = [self content:catelog];
    //: return index - begin;
    return index - begin;
}

//: - (NIMInputEmoticonCatalog *)emoticonWithIndex:(NSInteger)index {
- (ImageColor *)streetSmartIndexVideo:(NSInteger)index {
    //: NSInteger page = 0;
    NSInteger page = 0;
    //: NIMInputEmoticonCatalog *emoticon;
    ImageColor *emoticon;
...
- (NSInteger)tool:(ImageColor *)emoticonCatalog
{
...
- (void)change: (PageView *)pageView
    ...
    ImageColor *emticon = [self streetSmartIndexVideo:index];
...
- (void)should:(CityCenterWith*)emoticon independent_strong:(NSString*)catalogID{
    ...
    if ([self.delegate respondsToSelector:@selector(ting:userText:frame:)]) {


        //: if(emoticon.gif.length>0){
        if(emoticon.gif.length>0){
            //: [self.delegate selectedGifEmoticon: emoticon.filename];
            [self.delegate enable: emoticon.filename];
        //: }else{
        }else{
//            [self.delegate selectedEmoticon:emoticon.emoticonID catalog:catalogID description:emoticon.tag];
                    ...
                    if (emoticon.type == NIMEmoticonTypeUnicode) {
                        ...
                        [self.delegate ting:emoticon.emoticonID userText:catalogID frame:emoticon.unicode];
                    //: } else {
                    } else {
                        //: [self.delegate selectedEmoticon:emoticon.emoticonID catalog:catalogID description:emoticon.tag];
                        [self.delegate ting:emoticon.emoticonID userText:catalogID frame:emoticon.tag];
                    }
        }
...
- (void)someName:(id)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(someName:)]) {
        //: [self.delegate didPressSend:sender];
        [self.delegate someName:sender];
    }
}
...
- (void)controlled:(id)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(controlled:)]) {
        //: [self.delegate didPressDelete:sender];
        [self.delegate controlled:sender];
    }
}
...
- (void)setCurrentCatalogData:(ImageColor *)currentCatalogData{
    //: _currentCatalogData = currentCatalogData;
    _currentCatalogData = currentCatalogData;
    //: [self.emoticonPageView scrollToPage:[self pageIndexWithEmoticon:_currentCatalogData]];
    [self.emoticonPageView encounter:[self content:_currentCatalogData]];
}
...
- (NSArray *)allEmoticons{
    ...
    for (ImageColor *catalog in self.totalCatalogData) {
...
//: #pragma mark - Get
#pragma mark - Get
//: - (BusyInputEmoticonTabView *)tabView
- (ViewControl *)tabView
{
    //: if (!_tabView) {
    if (!_tabView) {
        //: _tabView = [[BusyInputEmoticonTabView alloc] initWithFrame:CGRectMake(0, 0, self.nim_width, 0)];
...
//        [_tabView.sendButton addTarget:self action:@selector(didPressSend:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [_tabView.sendButton addTarget:self action:@selector(controlled:) forControlEvents:UIControlEventTouchUpInside];
