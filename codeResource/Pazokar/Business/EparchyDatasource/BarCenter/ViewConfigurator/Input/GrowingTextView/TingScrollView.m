...
- (CGSize)intrinsicContentSize
{
    ...
    return [self mark:self.nim].size;
}
...
- (void)setMinNumberOfLines:(NSInteger)minNumberOfLines
{
    ...
    self.minHeight = [self big:minNumberOfLines];
...
- (void)setMaxNumberOfLines:(NSInteger)maxNumberOfLines
{
    ...
    self.maxHeight = [self big:maxNumberOfLines];
...
- (CGFloat)big:(NSInteger)line
{
    ...
    CGFloat height = self.nim.height - (textViewMargin + self.textView.contentInset.top + self.textView.contentInset.bottom);
...
- (void)tagComplete{
    ...
    if (newScrollViewFrame.size.height <= self.maxHeight && _previousTextHeight == 0) {
        ...
        if(oldScrollViewFrame.size.height != newScrollViewFrame.size.height) {
            ...
            if ([self.textViewDelegate respondsToSelector:@selector(tipScreen:)]) {
                //: [self.textViewDelegate willChangeHeight:newScrollViewFrame.size.height];
                [self.textViewDelegate tipScreen:newScrollViewFrame.size.height];
            }
    ...
    if (oldScrollViewFrame.size.height != newScrollViewFrame.size.height && [self.textViewDelegate respondsToSelector:@selector(dismissed:)]) {
        //: [self.textViewDelegate didChangeHeight:newScrollViewFrame.size.height];
        [self.textViewDelegate dismissed:newScrollViewFrame.size.height];
    }
...
- (CGRect)mark:(CGSize)contentSize
{
    //: CGSize selfSize;
    CGSize selfSize;
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(rangeUser:title:)]) {
        //: return [self.textViewDelegate shouldChangeTextInRange:range replacementText:text];
        return [self.textViewDelegate rangeUser:range title:text];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(mix:showAssemble:)]) {
        //: return [self.textViewDelegate shouldInteractWithURL:URL inRange:characterRange];
        return [self.textViewDelegate mix:URL showAssemble:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithTextAttachment:(NSTextAttachment *)textAttachment inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(handle:pressedSend:)]) {
        //: return [self.textViewDelegate shouldInteractWithTextAttachment:textAttachment inRange:characterRange];
        return [self.textViewDelegate handle:textAttachment pressedSend:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(drafts:)]) {
        //: [self.textViewDelegate textViewDidBeginEditing:self];
        [self.textViewDelegate drafts:self];
    }
}
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(limitManager:)]) {
        //: [self.textViewDelegate textViewDidChangeSelection:self];
        [self.textViewDelegate limitManager:self];
    }
}
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(batteried:)]) {
        //: [self.textViewDelegate textViewDidEndEditing:self];
        [self.textViewDelegate batteried:self];
    }
}
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(bringHome:)]) {
       //: return [self.textViewDelegate textViewShouldBeginEditing:self];
       return [self.textViewDelegate bringHome:self];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textViewShouldEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(alongRecordRestore:)]) {
        //: return [self.textViewDelegate textViewShouldEndEditing:self];
        return [self.textViewDelegate alongRecordRestore:self];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(valued:)]) {
        //: [self.textViewDelegate textViewDidChange:self];
        [self.textViewDelegate valued:self];
    }
    //: [self fitToScrollView];
    [self tagComplete];
}
